...
export const SmoothScrollProviderWithouDestroy = ({ children, options, containerRef }) => {
  const { height } = useResizeObserver({ ref: containerRef })
  const [isReady, setIsReady] = useState(false)
  const LocomotiveScrollRef = useRef(null)
  const { route } = useRouter()
...
SmoothScrollProviderWithouDestroy.displayName = 'SmoothScrollProviderWithouDestroy'
